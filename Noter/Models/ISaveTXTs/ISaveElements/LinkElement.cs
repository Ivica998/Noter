...
namespace Noter.Models.ISaveTXTs.ISaveElements
{
    public class LinkElement : ElementBase
    {
        ...
        public override string FormatSave(FileSaver fs, int depth)
        {
            ...
            string term = $" {depth + 1}#\n"; // prop ima depth + 1
            sb.Append(SavingHelper.Indent(depth) + "LinkElement:" + "\n");
            sb.Append(SavingHelper.Indent(depth + 1) + "LinkGuid:\n" + LinkGuid.ToString().Escape() + term);
        ...
        public override void ParseLoad(string body, int depth)
        {
            ...
            LinkGuid = Guid.Parse(props[counter++].AfterFirst('\n').Unescape());
